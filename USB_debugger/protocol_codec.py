...
from protocol_definitions import SOF1, SOF2, MsgType, FOC_USB_DEBUG_SIGNAL_LIST
...
class DecodedPacket:
    msg_type: MsgType
    data: DecodedLogPayload | DecodedPIDPayload | None

@dataclass
class DecodedLogPayload:
    timestamp: int
    sample_count: int
    signal_count: int
...
class ProtocolCodec:
    def __init__(
        ...
        self.msg_type_enum = msg_type_enum
        self.running = False
    ...
    def extract_packets(self, buffer: bytearray) -> list[Packet]:
        packets: list[Packet] = []
        ...
        while True:
            ...
            sof_index = buffer.find(bytes([self.sof1, self.sof2]))
            if sof_index == -1:
                buffer.clear()
                break
    ...
    def decode_packet(self, packet: Packet) -> DecodedPacket | None:
        try:
            pkt_type = self.msg_type_enum(packet.msg_type)
        except ValueError:
            print(f"Unknown packet type: {packet.msg_type}")
            return None

        match pkt_type:
            case self.msg_type_enum.MSG_LOG_DATA:
                decoded = self.decode_log_payload(packet.payload)
            case self.msg_type_enum.MSG_PID_REPLY:
                decoded = self.decode_pid_payload(packet.payload)
            case _:
    ...
    def decode_log_payload(self, payload: bytes, log_mask: int | None = None) -> DecodedLogPayload | None:
        if log_mask is None:
            log_mask = self.log_mask
    ...
    def decode_pid_payload(self, payload: bytes) -> DecodedPIDPayload | None:
        if len(payload) != 13:
        ...
        print(f"Decoded PID_REPLY: controller_id={controller_id}, kp={kp}, ki={ki}, kd={kd}")

        return DecodedPIDPayload(
    ...
    def build_packet(self, packet: Packet) -> bytes:
        ...
        return struct.pack("<BBBH", self.sof1, self.sof2, msg_type, payload_length) + packet.payload
    
    def build_text_command_packet(self, command_str: str) -> bytes:
        command_str = command_str.strip()

        if not command_str:
            return self.build_packet(Packet(self.msg_type_enum.MSG_START_LOG, b""))
        ...
        if cmd == "stop":
            self.running = False
            return self.build_packet(Packet(self.msg_type_enum.MSG_STOP_LOG, b""))
        ...
        if cmd == "setmask":
            ...
            return self.build_packet(Packet(self.msg_type_enum.MSG_SET_MASK, struct.pack("<I", new_mask)))
        ...
        if cmd == "getpid":
            ...
            return self.build_packet(Packet(self.msg_type_enum.MSG_GET_PID, struct.pack("<B", controller_id)))
        ...
        if cmd == "setpid":
            ...
            return self.build_packet(Packet(self.msg_type_enum.MSG_SET_PID, struct.pack("<Bfff", controller_id, kp, ki, kd)))
